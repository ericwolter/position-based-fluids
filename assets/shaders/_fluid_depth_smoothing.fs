...
void SampleDensityAndGradient(vec3 worldPos, out float density, out vec3 gradient)
{
    ...
    density = 0.0;
    gradient = vec3(0);
	vec3 gradient2 = vec3(0);
    int count = 0;
    
    // Compute Grid position
    ivec3 gridCell = ivec3(worldPos / smoothLength);

    float GRAD_SPIKY_KERNEL = 7792098820.0;
    float POLY6_KERNEL = 19877804900000.0;
            ...
            for (int ix = -1; ix <= +1; ix++)
            {
                ...
                while (partIdx != -1)
                {
                    ...
                    if (r_2 < h_2)
                    {
                        ...
                        gradient2 += -GRAD_SPIKY_KERNEL * h_r_diff * h_r_diff * (delta/r_length);
    ...
    density  *=  POLY6_KERNEL; // poly6Factor;
	gradient = -gradient2;
}
...
void main()
{
    ...
    float TargetDensity = 19877804900000.0 / 70000000000.0;

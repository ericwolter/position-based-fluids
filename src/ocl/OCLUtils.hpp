...
public:
     ...
     */
    vector<cl::Platform> getPlatforms(void) const;

    /**
     *  \brief  Selects one platform.
     */
    cl::Platform selectPlatform(void) const;

    /**
     *  \brief  Return a context from properties.
     */
    cl::Context createContext(cl_context_properties properties[], const cl_device_type deviceType = CL_DEVICE_TYPE_ALL) const;

    /**
     *  \brief  Return a context from platform.
     */
    cl::Context createContext(const cl::Platform &platform, const cl_device_type deviceType = CL_DEVICE_TYPE_ALL) const;

    /**
     *  \brief  Returns a device vector from context.
     */
    vector<cl::Device> getDevices(const cl::Context &context) const;
     ...
     */
    cl::Device selectDevice(const cl::Context &context) const;

    /**
     *  \brief  Selects one device from platform.
     */
    cl::Device selectDevice(const cl::Platform &platform, const cl_device_type deviceType = CL_DEVICE_TYPE_ALL) const;
    ...
    cl::Program createProgram(const string &source, const cl::Context &context, const vector<cl::Device> &devices, const string compileOptions = "") const;

    cl::Program createProgram(const string &source, const cl::Context &context, const cl::Device &device, const string compileOptions = "") const;

    /**
     *  \brief  Returns a kernel built from a program.
     */
    cl::Kernel createKernel(const string &programName, const cl::Program &program) const;

    /**
     *  \brief  Returns a kernel vector built from a program.
     */
    vector<cl::Kernel> createKernels(cl::Program &program) const;
     ...
     */
    void printPlatformInfo(const cl::Platform &platform) const;

    /**
     *  \brief  Prints information about devices.
     */
    void printDeviceInfo(const cl::Device &device) const;
};
